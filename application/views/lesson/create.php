<form action="<?= site_url('syllabus/lesson/save/' . (empty($selectedCourse) ? '' : $selectedCourse['id'] ) . '?redirect=' . get_url_param('redirect')) ?>" method="POST" id="form-lesson">
    <?= _csrf() ?>
	<div class="card mb-3">
        <div class="card-body">
			...
			<div class="row">
						...
						<?php if(empty($selectedCourse)): ?>
							<select class="form-control select2" id="course" name="course" data-placeholder="Select course" required>
								...
								<?php foreach ($courses as $course): ?>
									<option value="<?= $course['id'] ?>"<?= set_select('course', $course['id']) ?>>
										<?= $course['course_title'] ?> - <?= $course['curriculum_title'] ?>
									</option>
								<?php endforeach; ?>
							</select>
						<?php else: ?>
							<input type="text" class="form-control" value="<?= $selectedCourse['course_title'] ?>" id="course" readonly>
				...
				<div class="col-sm-6">
					<div class="form-group">
						...
						<input type="text" class="form-control" id="lesson_title" name="lesson_title" required maxlength="100"
							   value="<?= set_value('lesson_title') ?>" placeholder="Lesson title">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="description" name="description" maxlength="500"
                          placeholder="Enter lesson description"><?= set_value('description') ?></textarea>
	...
	<div class="card mb-3">
		<div class="card-body">
			...
			<?php $fileInput = set_value('file_input') ?>
				...
				<div class="upload-content-wrapper">
					<div class="d-flex flex-row align-items-center p-4 mb-2" style="border: 1px dashed #aaaaaa;">
						...
						<div class="text-left mr-4">
							...
							<p class="small text-muted mb-2 text-truncate upload-label-file" style="max-width: 300px">
