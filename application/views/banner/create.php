<form action="<?= site_url('banner/save') ?>" method="POST" enctype="multipart/form-data" id="form-banner">
    <?= _csrf() ?>
    <div class="card mb-3">
        <div class="card-body">
            <h5 class="card-title">Create New Banner</h5>

            <div class="form-group">
                <label for="title">Title</label>
                <input type="title" class="form-control" id="title" name="title" maxlength="500" required
                        value="<?= set_value('title') ?>" placeholder="menu name">
                <?= form_error('title') ?>
            </div>
            <div class="form-group">
                <label for="caption">Caption</label>
                <input type="caption" class="form-control" id="caption" name="caption" maxlength="500" required
                        value="<?= set_value('caption') ?>" placeholder="menu name">
                <?= form_error('caption') ?>
            </div>
            <div class="form-group">
                <label>Banner</label>
            ...
            <div class="d-flex justify-content-between mt-3">
                ...
                <button type="submit" class="btn btn-success mr-2" data-toggle="one-touch" data-touch-message="Saving...">Save Banner</button>
            </div>
        </div>
    </div>
</form>
