<form action="<?= site_url('master/menu/update/' . $menu['id']) ?>" method="POST" enctype="multipart/form-data" id="form-menu">
    <?= _csrf() ?>
    <div class="card mb-3">
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="menu_name" class="form-control" id="menu_name" name="menu_name" maxlength="500" required
                        value="<?= set_value('menu_name', $menu['menu_name']) ?>" placeholder="menu name">
            ...
            <div class="form-group">
                ...
                <select class="custom-select" id="link_type" name="link_type">
                    ...
                    <option value="INTERNAL" <?= set_select('link_type', 'INTERNAL', $menu['link_type'] == 'INTERNAL' ) ?>>INTERNAL</option>
                    <option value="EXTERNAL" <?= set_select('link_type', 'EXTERNAL', $menu['link_type'] == 'EXTERNAL' ) ?>>EXTERNAL</option>
                </select>
                <?= form_error('link_type') ?>
            </div>
            <div class="form-group" style="<?= $menu['link_type'] == 'INTERNAL' ? '': 'display:none' ?> " id="type-internal">
                ...
                <select class="custom-select" id="url_page" name="url_page">
                    ...
                    <option value="<?= base_url().$page['url'] ?>" <?= set_select('url_page', base_url().$page['url'], $menu['url'] == base_url().$page['url'] ) ?>><?= $page['page_name'] ?></option>
                    <?php endforeach; ?>
                </select>
                <?= form_error('url') ?>
            </div>
            <div class="form-group" style="<?= $menu['link_type'] == 'EXTERNAL' ? '': 'display:none' ?> " id="type-external">
